# Garage & Power Electronics — AI Platform Deployment Status
...
The Coo-Cah AI Platform hosts all AI inference services consumed by the Garage & Power Electronics Factory MES. At Gate 3, stub/mock endpoints are deployed and reachable, allowing full integration testing without live production data.
...
| `/api/v1/ai/load-bank-test-yield`           | POST   | Load bank test pass/fail yield prediction by SKU          | ✅ Active      | ⏳ Pending P1  |
...
| `/api/v1/ai/winding-quality-predict`        | POST   | Transformer winding quality prediction (Phase 2)          | ✅ Active      | ⏳ Pending P2  |
...
| `/api/v1/ai/inverter-efficiency-forecast`   | POST   | Inverter efficiency trend prediction (product twin)       | ✅ Active      | ⏳ Pending P1  |
...
|--------------------------------|------------------------------------------------|--------------------------|
| `svc-mes-ai-stub-power`        | SMT predict, load bank test yield, OEE, AMR    | AI Platform / IT Ops     |
| `svc-dt-feed-stub-power`       | Machine state, simulation run, product twin    | Digital Manufacturing    |
| `svc-winding-predict-stub`     | Winding quality, predictive maintenance        | Quality Engineering      |
...
| MES → Load Bank Test Yield API       | Happy path, SKU-level yield flag logic         | 2025-Q3       | ✅ Pass  |
